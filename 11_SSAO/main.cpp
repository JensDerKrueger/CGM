...
public:
  ...
  void renderScreenSpaceAmbientOcclusion() {
    ...
    // TODO Task 1:
    // Configure and execute the SSAO full-screen pass.
    //
    // 1. Pass projectionMatrix to the shader. The shader creates test positions
    //    in view space and projects them back into screen coordinates.
    // 2. Pass the SSAO parameters radius, bias, intensity, sampleCount, and
    //    maxMipLevel. Start with the values suggested in the exercise text.
    // 3. Bind normalBuffer and positionBuffer. The shader needs normals to
    //    orient the hemisphere and positions to compare depths.
    // 4. Bind fullscreenArray and draw one full-screen triangle.
    //
    // Until this is implemented the AO texture stays white because draw() clears
  ...
  void renderAmbientOcclusionBlurPass(const GLTexture2D& sourceBuffer, const Vec2& direction) {
    ...
    // TODO Task 3:
    // Execute one separable SSAO blur pass.
    //
    // 1. Pass direction to the shader. Use Vec2{1, 0} for the horizontal pass
    //    and Vec2{0, 1} for the vertical pass.
    // 2. Pass a depthFalloff value. Values around 12.0 are a reasonable start.
    // 3. Bind sourceBuffer as sourceAmbientOcclusionBuffer and bind
    //    positionBuffer.
    // 4. Draw the full-screen triangle.
    //
    // The position buffer is important: the blur should smooth AO along the same
    // surface, but avoid smearing dark AO over unrelated pixels at different
    // depths. Applying a 5-tap blur twice is much cheaper than a full 5 x 5 blur.
  ...
  virtual void draw() override {
    ...
    // TODO Task 1:
    // Call renderScreenSpaceAmbientOcclusion() here. The clear color above is
    // white, which corresponds to "no occlusion"; after your pass is active, the
    // shader will overwrite this with the estimated SSAO values.
    ...
    GL(glClear(GL_COLOR_BUFFER_BIT));
    // TODO Task 3:
    // Call renderAmbientOcclusionBlurPass(rawAmbientOcclusionBuffer,
    // Vec2{1.0f, 0.0f}) here after you have implemented the blur shader.
    tempAmbientOcclusionFramebuffer.unbind2D();

    // Fourth pass: vertical smoothing. This completes the separable blur and
    // writes the texture used by the final lighting pass. This justifies keeping
    // SSAO as a separate image-space pass: once the raw AO result exists as a
    // texture, it can be filtered before use.
    ambientOcclusionFramebuffer.bind(ambientOcclusionBuffer);
    GL(glViewport(0, 0, GLsizei(dim.width), GLsizei(dim.height)));
    GL(glClearColor(1.0f, 1.0f, 1.0f, 1.0f));
    GL(glClear(GL_COLOR_BUFFER_BIT));
    // TODO Task 3:
    // Call renderAmbientOcclusionBlurPass(tempAmbientOcclusionBuffer,
    // Vec2{0.0f, 1.0f}) here after you have implemented the blur shader.
