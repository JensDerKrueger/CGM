...
public:
  ...
  MyGLApp() :
  GLApp(800, 600, 1, "Exercise 12 - Deferred Shading"),
  ...
  void ensureGBufferSize(const Dimensions dim) {
    // TODO Task 1:
    // If the framebuffer size did not change, return immediately.
    // Otherwise allocate the four G-buffer textures with the new size:
    //   diffuseBuffer, specularBuffer, normalBuffer, positionBuffer
    // Each buffer should have four floating point components. Also resize the
    // depth renderbuffer and store the new size in gBufferSize.
  }
  ...
  void renderDeferredImage() {
    ...
    // TODO Task 3:
    // Pass the debug-toggle state and the light position to the deferred
    // shader. Bind the four G-buffer textures to four texture units using the
    // sampler names diffuseBuffer, specularBuffer, normalBuffer, and
    // positionBuffer.
  ...
  virtual void draw() override {
    ...
    // TODO Task 1:
    // First pass: bind the four G-buffer textures and the depth buffer to
    // gBuffer. Set the viewport to the framebuffer size, clear color alpha to
    // zero, clear color and depth, render the scene with renderSceneToGBuffer,
    // and unbind the framebuffer again.
